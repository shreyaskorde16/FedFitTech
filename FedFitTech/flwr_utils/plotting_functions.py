...
"""
# ------------------------------------------------------------------------
Created on Thu Oct 24 14:17:57 2024
...
def plot_all_sbj_using_line_chart(df: pd.DataFrame, 
                                  directory_name, 
                                  save_fig=True) -> plt:
...
def plot_one_sbj_using_line_chart(df: pd.DataFrame(), 
                                  client_num: int, 
    ...
    plt.figure(figsize=(12, 8))  # Adjust the figure size as needed
    plt.plot(df.iloc[:, client_num], label='Client_'+str(client_num))  # Change the index or name to select a different column
...
def plot_LMs_means_per_global_round(df: pd.DataFrame, 
                                    GLOBAL_ROUND: int, 
                                    directory_name,
                                    save_fig=True) -> plt:
    ...
    plt.plot(LMs_means_per_global_round, label='LMs means')  # Change the index or name to select a different column
...
def plot_GM_performance_after_FL_training(df: pd.DataFrame, 
                                          GLOBAL_ROUND: int,
                                          directory_name,
                                          save_fig=True) -> plt:
    """
...
def plot_multi_bar_chart(F1Score_dict_all: dict,
                        LEARNING_TYPES_LIST: list,
                        directory_name,
                        save_fig=True) -> plt:
    ...
    for i in range(values.shape[1]):  # Iterate over the 3 values per client
        bars = ax.bar(x + i * bar_width, values[:, i], width=bar_width, label=LEARNING_TYPES_LIST[i])
    ...
    line_handles = [
        Line2D([0], [0], color='blue', linestyle='--', linewidth=1.5, label=f'LL mean: {LL_mean_value:.2f}'),
        Line2D([0], [0], color='orange', linestyle='--', linewidth=1.5, label=f'CL mean: {CL_mean_value:.2f}'),
        Line2D([0], [0], color='green', linestyle='--', linewidth=1.5, label=f'FL mean: {FL_mean_value:.2f}')
    ]
    
    # Combine bar and line handles for a unified legend
    combined_handles = bar_handles + line_handles
    combined_labels = LEARNING_TYPES_LIST + [f'LL mean: {LL_mean_value:.2f}',
...
def plot_label_amount_as_window(training_dataloader_all,
    ...
    bars1 = plt.bar(x - bar_width/2, train_counts, bar_width, label='Training', color='skyblue', edgecolor='black')
    bars2 = plt.bar(x + bar_width/2, test_counts, bar_width, label='Testing', color='lightgreen', edgecolor='black')
    
    title = f"Label Distribution Comparison: Training vs. Testing for Client_{client_num}"
...
def plot_heat_map_of_table(label_based_result_table: pd.DataFrame,
    ...
    # Create the heatmap
    sns.heatmap(label_based_result_table, 
                ...
                cbar_kws={'label': 'F1 Score'})  # Add a color bar label
...
class advanced_plot():
    ...
    # Column names:
    columns = ['RA_x', 'RA_y', 'RA_z', 
               'RL_x', 'RL_y', 'RL_z',
               'LL_x', 'LL_y', 'LL_z',
               'LA_x', 'LA_y', 'LA_z']
        
    
    def sensor_based_analysis(user_test_features_all: [np.array], 
        ...
        """
        4 sonsors are subplotted and Grounth Truth vs Predicted Labels can be analysed via colour map.
        """
        ...
        def plot_sensor_data(ax, sensor_data, sensor_name):
            for i in range(num_windows):
                ...
                ax.plot(range(start, end), sensor_data[start:end, 0], label=f"{sensor_name}_x" if i == 0 else "", color="blue")
                ax.plot(range(start, end), sensor_data[start:end, 1], label=f"{sensor_name}_y" if i == 0 else "", color="orange")
                ax.plot(range(start, end), sensor_data[start:end, 2], label=f"{sensor_name}_z" if i == 0 else "", color="green")
                if i < num_windows - 1:
                    ax.axvline(end, color="black", linestyle="--", linewidth=0.5)
            ax.set_xlim(0, data.shape[0])
        
        fig, axs = plt.subplots(5, 1, figsize=(35, 12), gridspec_kw={'height_ratios': [1, 1, 1, 1, 0.5]})
        ...
        real_label_array = np.array([all_labels.index(label) for label in real_labels[:num_windows]])
        pred_label_array = np.array([all_labels.index(label) for label in pred_labels[:num_windows]])
        ...
        for i in range(1, num_windows):
            axs[4].axvline(i * window_size, color="black", linestyle="--", linewidth=0.5)
        
        if separate_results:
            axs[4].axhline(0, color="black", linewidth=0.5)
            
        legend_handles = [Patch(color=label_colors[label], label=label) for label in all_labels]
        fig.legend(handles=legend_handles, loc='lower center', ncol=len(all_labels))
            
        
        fig.suptitle(f"{learning_type_name} - Ground Truth vs Predictions of Client_{client_num}", fontsize=36, fontweight='bold')
        
        if save_fig:
            file_path = os.path.join(directory_name, f"Ground Truth vs Predictions of Client_{client_num}")
    ...
    def sensor_based_analysis_with_all_learning_systems(user_test_features_all: [np.array], 
                              real_labels_all: list, 
        ...
        """
        4 sensors are subplotted, and Ground Truth vs Predicted Labels can be analyzed via color map.
        """
        ...
        real_labels = [reversed_labels_set[l] for l in real_labels_all[client_num]]
        pred_labels_LL = [reversed_labels_set[l] for l in pred_labels_all_LL[client_num]]
        pred_labels_CL = [reversed_labels_set[l] for l in pred_labels_all_CL[client_num]]
        pred_labels_FL = [reversed_labels_set[l] for l in pred_labels_all_FL[client_num]]
        ...
        real_label_array = np.array([all_labels.index(label) for label in real_labels[:num_windows]])
        pred_label_array_LL = np.array([all_labels.index(label) for label in pred_labels_LL[:num_windows]])
        pred_label_array_CL = np.array([all_labels.index(label) for label in pred_labels_CL[:num_windows]])
        pred_label_array_FL = np.array([all_labels.index(label) for label in pred_labels_FL[:num_windows]])

        # Combine arrays for the imshow plot
        combined_labels = np.vstack([real_label_array, pred_label_array_LL, pred_label_array_CL, pred_label_array_FL])
        ...
        axs[4].set_yticklabels(['Federated Learning', 'Centralized Learning', 'Local Learning', 'Ground Truth'], fontweight='bold')
        axs[4].set_xticks(range(0, num_windows * window_size, window_size))
        axs[4].set_xticklabels([])
        ...
        fig.suptitle(f"Ground Truth vs Predictions of Client_{client_num} Over All Learning Systems", fontsize=36, fontweight='bold')
    ...
    def interactive_analysis(user_test_features_all: [np.array], 
                             real_labels_all: list, 
                             pred_labels_all: list, 
                             client_num: int, 
                             window_size: int,
                             directory_name,
                             labels_set,
                             save_fig=False) -> go.Figure:
        """
        An interactive analysis of all sensors together in a line graph with advantages of zooming a specific range.
        """
        data = user_test_features_all[client_num]
        ...
        for i in range(num_windows):
            ...
            fig.add_annotation(x=midpoint, y=9, text=f"W{i}", showarrow=False, font=dict(color="black", size=10))
            fig.add_annotation(x=midpoint, y=-5, text=f"Real: {real_labels[i]}", showarrow=False, font=dict(color="blue", size=10))
            fig.add_annotation(x=midpoint, y=-7, text=f"Pred: {pred_labels[i]}", showarrow=False, font=dict(color="red", size=10))
...
class common_plots():
    ...
    def plot_clients_performance_after_training(final_fscores, 
                                                training_round, 
                                                directory_name=None, 
                                                save_fig=False,
        ...
        title = f"{learning_type_name} - Performance after {training_round} Training Round"
    
        # Create a figure with a larger size
        plt.figure(figsize=(18, 6))  # Width: 18, Height: 6
    
        # Create the bar chart
        bars = plt.bar(common_plots.categories, final_fscores, color='skyblue', edgecolor='black', width=0.6)
