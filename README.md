...
**2.2 Model:** <p align="justify"> [TinyHAR](https://publikationen.bibliothek.kit.edu/1000150216) is a lightweight deep learning model designed for Human Activity Recognition (HAR). It combines convolutional layers, a transformer block with self-attention, fully connected layers, and an LSTM to efficiently extract and model features from sensor data. The convolutional layers capture local patterns, the transformer block enables cross-channel feature interaction, and the fully connected layer fuses these features. The LSTM then processes these features to capture long-term temporal dependencies, followed by a self-attention layer that dynamically recalibrates feature importance over time. This architecture achieves competitive performance with fewer parameters, making it suitable for resource-constrained devices.</p>
