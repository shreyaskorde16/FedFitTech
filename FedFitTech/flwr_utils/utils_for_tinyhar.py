"""
# ------------------------------------------------------------------------
# Coded for preparing WEAR dataset to run TinyHAR model in Federated Learning system by: Zeyneddin Oz
...
def extract_number(file_path: os.path) -> int:
    ...
    number = int(base_name.split('_')[1].split('.')[0])  # Extract the number after 'sbj_' and convert it to an integer
    
    return number


def load_data(user_path: os.path, 
              keep_NULL=True) -> [np.array, 
                                  np.array]:
    """
    ...
    user_df[X_columns] = pd.DataFrame(StandardScaler().fit_transform(user_df[X_columns]), 
                                      columns=X_columns)    
...
def manual_data_split(X_features: np.array, 
                      y_labels: np.array, 
                      SPLIT_SIZE=0.2) -> [np.array,
                                          np.array,
                                          np.array, 
                                          np.array]:
    ...
    while start_idx < len(y_labels):
        ...
        X_features = np.delete(X_features, np.s_[start_idx:start_idx + split_index], axis=0)
...
def take_most_common_label_in_a_window(arr: torch.Tensor, 
...
def generate_dataloaders(train_features: np.array, 
                         train_labels: np.array, 
                         test_features: np.array, 
                         test_labels: np.array,
                         batch_size=32, 
                         sequence_length=50) -> [torch.utils.data.dataloader.DataLoader,
    ...
    n_train_samples = (train_features_tensor.shape[0] // sequence_length) * sequence_length
    n_test_samples = (test_features_tensor.shape[0] // sequence_length) * sequence_length
    ...
    train_features_tensor = train_features_tensor.reshape(-1, 1, sequence_length, train_features_tensor.shape[1])
    test_features_tensor = test_features_tensor.reshape(-1, 1, sequence_length, test_features_tensor.shape[1])

    # ############################### Preparing Label ###############################
    # Trim labels accordingly. Take each windows and from each windows select the most common label:   
    train_labels_tensor = take_most_common_label_in_a_window(train_labels_tensor, sequence_length)
    test_labels_tensor = take_most_common_label_in_a_window(test_labels_tensor, sequence_length)
...
def add_pred_to_all_data(user_labels, 
                         user_test_labels, 
                         user_pred_labels: list) -> list:
...
class training_functions():
    ...
    def train_model_federated(model, 
                              trainloader, 
                              num_epochs, 
                              opt, 
                              learning_rate, device):
...
class evaluation_functions():
    ...
    def evaluate_model(model, 
        ...
        if result_print:
            print(f"{'*'*10} Client ID {cfg.sub_id} {'*'*10}\n" # print(f"*"*10,f"{client}",f"*"*10)
    ...
    def get_ground_truth_and_predictions(model, 
                                         testloader, DEVICE):
    ...
    def get_label_based_results(all_labels, 
                                all_predictions, 
                                reversed_labels_set):
        ...
        res = classification_report(all_labels, all_predictions, zero_division=0, output_dict=True)
        ...
        for numeric_label, label_name in reversed_labels_set.items():
            if str(numeric_label) in res:  # Check if the label exists in classification report
                ...
                label_accuracy = (label_predictions == label_labels).float().mean().item() if label_labels.size(0) > 0 else 0.0
        
                # Calculate loss for the current label if there are predictions
                if label_labels.size(0) > 0:
                    label_logits = F.one_hot(label_predictions, num_classes=len(reversed_labels_set)).float()
