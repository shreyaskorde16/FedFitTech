...
class FlowerClient(NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if context_early_stop["has_converged"]:
            ...
            if context_early_stop["print_status"] == False:
                ...
                msg_wth_stp_rnd = f"Config records for Client Id {self.cfg.sub_id}: " \
                ...
                if not os.path.exists(fit_log_file):
                    with open(fit_log_file, "w") as f:  # "w" mode creates the file if missing
            ...
            loss, accuracy = training_functions.train_model_federated(self.net, 
                                                                        self.trainloader, 
    ...
    def evaluate(self, parameters, config):
        ...
        if server_round == self.cfg.GLOBAL_ROUND:
            real_labels_all = []
            pred_labels_all = []
            all_labels, all_predictions = evaluation_functions.get_ground_truth_and_predictions(model=self.net, 
                                                                                            testloader=self.valloader, 
                                                                                            DEVICE=self.DEVICE)
            real_labels_all.append(all_labels)
            pred_labels_all.append(all_predictions)
            labelwise_result_dict = evaluation_functions.get_label_based_results(all_labels, all_predictions, self.cfg.reversed_labels_set)
            complex_labelwise_result_dict = json.dumps(labelwise_result_dict) # Converts dict to str
            metrics =  {
                "Validation loss": "{:.4f}".format(loss),
                "Validation Accuracy": "{:.2f}".format(accuracy),
                "Validation Precision": "{:.4f}".format(precision),
        ...
        if self.cfg.Early_stopping in ['true', 'yes', '1']:
            ...
            if len(f1_scores) > self.patience:
                ...
                print(f"Client id {self.cfg.sub_id} f1_diff is {round(max(f1_scores_window) - min(f1_scores_window), 2)} and threshold is {self.threshold}")
                context_early_stop["context_best_val_f1_score"] = fscore
                if round(max(f1_scores_window) - min(f1_scores_window), 2) < self.threshold:
                    context_early_stop["has_converged"] = True
                    print(f"+++++++++++++++++++++++++++++ Early stopping triggerd for client {self.cfg.sub_id} +++++++++++++++++++++++++++++++")
