...
def load_data_for_client(cfg, user_num):
    ...
    for path in csv_file_paths:
        ...
        if sub == user_num:
            ...
            cfg.sub_id = sub
        if  sub >= len(csv_file_paths):   # as sub_0 is included so sub_0 to sub_23, len 24
    ...
    user_train_features_all, user_train_labels_all, user_test_features_all, user_test_labels_all = [], [], [], [] 
    ...
    X_user_train_features, y_user_train_labels, X_user_test_features, y_user_test_labels = manual_data_split(
        X_user_features, 
        y_user_labels) 
    ...
    trainloader, testloader = generate_dataloaders(
        user_train_features_all[0], user_train_labels_all[0],
        user_test_features_all[0], user_test_labels_all[0],
...
def get_net_and_config():
    ...
    hyperparameters = {'LEARNING_RATE': cfg.LEARNING_RATE,
    ...
    net = TinyHAR(input_shape=(cfg.BATCH_SIZE, 1, cfg.WINDOW_SIZE, cfg.NUM_FEATURES), 
                number_class= cfg.NUM_CLASS, filter_num=cfg.NUM_FILTERS)
...
def get_model_plot_directory(plt_dir = True, model_dir = True, csv_dir = True, config = None, server_round = "_"):
    ...
    if plt_dir:
        ...
        os.makedirs(plt_directory_name, exist_ok=True)  # Creates directory if it doesn't exist
    if model_dir:
